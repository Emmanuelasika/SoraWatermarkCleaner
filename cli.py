...
Examples:
  ...
  python batch_process.py -i /path/to/input -o /path/to/output --pattern "*.{mp4,mov,avi}"
    ...
    if not input_folder.exists():
        console.print(
            f"[bold red]❌ Error: Input folder does not exist:[/bold red] {input_folder}",
            style="red"
        )
        sys.exit(1)
    ...
    if not input_folder.is_dir():
        console.print(
            f"[bold red]❌ Error: Input path is not a directory:[/bold red] {input_folder}",
            style="red"
        )
        sys.exit(1)
    ...
    try:
        processor = BatchProcessor(input_folder, output_folder, args.pattern)
